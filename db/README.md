...
| 短文本 / 主键 | `VARCHAR(n)` |
| JSON | `TEXT` |
|  surrogate 主键 | 避免方言自增；审计/日志用自然键组合 `PRIMARY KEY (...)`，编译任务用 `run_id VARCHAR` |
...
| virbius-compiler | `virbius-compiler.db` | `virbius-compiler/src/main/resources/db/schema.sql` |
| virbius-core | `virbius-core.db` | `virbius-core/db/schema.sql` |
| virbius-gateway-agent | `virbius-gateway-agent.db` | `virbius-gateway-agent/db/schema.sql` |
...
| tenants | `tb_tenants` |
| bundles | `tb_bundles` |
| rules_current | `tb_rules_current` |
| rule_history | `tb_rule_history` |
| access_list | `tb_access_list` |
| cache_meta | `tb_cache_meta` |
| rule_cache_entry | `tb_rule_cache_entry` |
| audit_events | `tb_audit_events` |
| compile_runs | `tb_compile_runs` |
| edge_manifest | `tb_edge_manifest` |
| edge_rule_bin | `tb_edge_rule_bin` |
| agent_evaluate_log | `tb_agent_evaluate_log` |
...
- `tb_audit_events` — 审计事件（与 jsonl 可并存）
