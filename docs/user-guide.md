...
本文面向 **App / 服务端 / Rust 客户端** 集成方，说明如何在业务中接入 Virbius 大模型防控能力。运营配置规则请使用 Admin 运营台（`ops.html`）及 [seed-api.md](./seed-api.md)；本文聚焦 **运行时集成**。
...
| [seed-api.md](./seed-api.md) | 本地 PoC、运营 API 与 curl 示例 |
| [repo-layout.md](./repo-layout.md) | 仓库布局与本地启动 |
...
let edge = VirbiusEdge::init(EdgeInitConfig {
    ...
    app_id: "beta".into(),
    edge_api_key: Some("vrb_tk_...".into()), // Control 开启 VIRBIUS_API_KEY_AUTH_ENABLED 时必填（tenant_viewer+）
...
### 3.2.1 Control API Key 鉴权（方案 B+）

Control 可通过 `VIRBIUS_API_KEY_AUTH_ENABLED=true` 开启统一 Bearer（默认关闭）。同一套 `tb_tenant_api_credential` 保护 Admin、Edge 拉取与 legacy tenants API：

| 项 | 说明 |
|----|------|
| Header | `Authorization: Bearer <api_key>` 或 `X-Virbius-Api-Key` |
| Scope | key 绑定 **tenant_id**；path 中 `{tenantId}` 须一致（`platform_admin` 的 `*` 除外） |
| Edge 拉取 | 须 `tenant_viewer` 及以上；PoC dev key：`vrb_tk_dev_viewer_default` |
| 运营写操作 | 须 `tenant_admin` 及以上；PoC：`vrb_tk_dev_admin_default` |
| 租户 CRUD | 须 `platform_admin`；PoC：`vrb_tk_dev_platform` |
| 签发 | `POST /api/v1/admin/tenants/{tenantId}/api-credentials`（响应含一次性 `api_key`） |
| 吊销 | `POST .../api-credentials/{credentialId}/revoke` |
...
| `VIRBIUS_EDGE_API_KEY` | Bearer；Control `VIRBIUS_API_KEY_AUTH_ENABLED=true` 时必填（通常用 `tenant_viewer` key） |
...
| 变量 | 说明 |
|------|------|
| `VIRBIUS_API_KEY_AUTH_ENABLED` | `true` 开启 Admin / Edge / tenants API Bearer（默认 `false`） |
| `VIRBIUS_DATA_DIR` | 产物与 SQLite 目录，默认 `./data` |

PoC 联调示例见 [seed-api.md §6.5](./seed-api.md)。
...
PoC curl 见 [seed-api.md §7](./seed-api.md)。
...
**运营台排障**：http://127.0.0.1:8080/ui → **审计中心**，输入 `trace_id` 可查看各层 audit 与 allow 日志；API：`GET /api/v1/admin/tenants/{tenant}/audit/trace/{traceId}`（详见 [seed-api.md](./seed-api.md)）。
...
- Edge manifest：`manifest_version: "1"`（字段见 [seed-api.md §6](./seed-api.md)）
- 架构与接口概览：[DESIGN.md](./DESIGN.md)

如有集成问题，可先对照 `cargo run --example rust_client_demo` 与 [seed-api.md](./seed-api.md) 中的 curl 行为是否一致。
