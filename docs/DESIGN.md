...
**已定约束**（[POC-SEED-API.md](POC-SEED-API.md)）：
...
| **对外 HTTP** | **单一监听端口**（PoC `:8080`）；**单一 OpenAPI**：[POC-SEED-API.md](POC-SEED-API.md) |
...
| **`rollout_state`** | **R1+ 运营真源**：`draft` \| `disabled` \| `dry_run` \| `canary` \| `full`（[POC-SEED-API.md](POC-SEED-API.md)） |
...
> **运营放量**见 [POC-SEED-API.md](POC-SEED-API.md)（`rollout_state`）；本节 **`enforce_mode`** 为执行面语义（ActionMerge）。
...
运营台：http://127.0.0.1:8080/ui → 左侧导航「规则」（云/管/端）与「**策略上线**」（`rollout_state` 放量；见 [POC-SEED-API.md](POC-SEED-API.md) §8.3）。契约：**Admin API** 以 [POC-SEED-API.md](POC-SEED-API.md)（`/api/v1/admin/...`）为准；gateway-agent `vars` 优先语义见同目录 [user-guide.md §4](user-guide.md)。
...
| enforce 晋升 | `rollout_state`：`dry_run → canary → full`（**禁止** dry_run→full）；租户门禁/阶梯见 [POC-SEED-API.md](POC-SEED-API.md)；可**不重编译**正文 |
...
| **virbius-control** | Java + 静态运营台 | 规则真源 API + 运营台；`rollout_state` 策略上线（[POC-SEED-API.md](POC-SEED-API.md)）；**单进程**（§7.1.1） |
...
MVP **包含** Registry 放量状态与 engine 快照；**门禁 / 阶梯自动化**见 [POC-SEED-API.md](POC-SEED-API.md) R2（Flink 自动晋升留 Phase 3）。
...
**PoC 过渡 API**（R1 前）：`PATCH .../status` + `PATCH .../runtime`（映射见 [POC-SEED-API.md](POC-SEED-API.md)）。
...
**规则放量（`rollout_state`，可与内容发布解耦）** — 详见 [POC-SEED-API.md](POC-SEED-API.md)：
...
PoC 非 allow 事件 publish → Redis Stream → control **AuditIngest** → **`tb_audit_events`**；**allow 不写 DB**。全链路排障：运营台 **「审计中心」** 按 `trace_id` 合并查询（[POC-SEED-API.md](POC-SEED-API.md)）。
...
| v2.7.1 | 2026-05-20 | 增补 MVP 契约说明（见 [POC-SEED-API.md](POC-SEED-API.md)、[user-guide.md](user-guide.md)） |
...
| v2.20 | 2026-05-20 | **rollout_state** 运营放量定稿（[POC-SEED-API.md](POC-SEED-API.md)）；body→draft；禁止 dry_run→full；租户级门禁/阶梯；audit Redis/Kafka ingest |
| v2.20 | 2026-05-20 | **§8.5.0.1** 累计规则 CounterStore 设计（本文 §8.5.0.1） |
| v2.22 | 2026-05-20 | **§7.1.1 / §8.5.0**：运营台为静态 **`ops.html`**（左侧导航 + 策略上线）；整包 Bundle 发布标 Legacy；与 [POC-SEED-API.md](POC-SEED-API.md) 对齐 |
