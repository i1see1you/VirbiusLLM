...
|------|------|
| 文档版本 | v2.21 |
| OpenSpec | [docs/openspec/MVP-OPENSPEC.md](openspec/MVP-OPENSPEC.md)（MVP-1.12） |
...
PoC 已实现 **Control 直拉 manifest + 租户级 Bearer**（DESIGN §8.10.2.5a、MVP-OPENSPEC §4.8）：
...
**`enforce_mode`**：写入 `rule_history`；**ActionMerge** 按每条命中规则的 `enforce_mode` 计算对外 **`effective_action`**（见 [rule-hit-merge.md](openspec/rule-hit-merge.md)）。
...
**定位**：`virbius-gateway` 不是某一种网关的实现，而是**管层安全数据面的统一集成层**——在 **APISIX** 与 **Kong**（MVP）之上，用同一套契约对接 **Registry 发布的编译产物** 与 **virbius-engine**。**Stretch**：纯 **OpenResty**（编译期拍平 + `access.lua`，见 [openresty-gateway.md](openspec/openresty-gateway.md)）。**不支持** Envoy、Wasm 主路径（见 §11.5.8）。
...
约 **70–85%** 逻辑在 `lib/`；各网关仅 **插件注册、取请求/写响应 API、配置挂载** 不同。详见 [openresty-gateway.md](openspec/openresty-gateway.md)。
...
| **Stretch（OpenResty）** | 编译期拍平 + `access.lua`；**非** MVP 验收；见 [openresty-gateway.md](openspec/openresty-gateway.md) |
...
**L3 合并**：engine **`PolicyMerger` / `ActionMerge`**（Java）；见 [rule-hit-merge.md](openspec/rule-hit-merge.md)。**已移除** `cloud_groovy_l3`。
...
PoC 非 allow 事件 publish → Redis Stream → control **AuditIngest** → **`tb_audit_events`**；**allow 不写 DB**。全链路排障：运营台 **「审计中心」** 按 `trace_id` 合并查询（[rule-rollout.md §5.8](openspec/rule-rollout.md)）。
