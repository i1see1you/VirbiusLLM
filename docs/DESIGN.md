...
**状态**：设计冻结，**PoC 代码尚未实现**。详见 **§8.5.0.2（本文）**。
...
**定位**：`virbius-gateway` 不是某一种网关的实现，而是**管层安全数据面的统一集成层**——在 **APISIX** 与 **Kong**（MVP）之上，用同一套契约对接 **Registry 发布的编译产物** 与 **virbius-engine**。**Stretch**：纯 **OpenResty**（编译期拍平 + `access.lua`，见 [../virbius-gateway/README.md](../virbius-gateway/README.md)）。**不支持** Envoy、Wasm 主路径（见 §11.5.8）。
