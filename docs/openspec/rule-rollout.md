...
各执行面（engine、gateway-agent、edge）**不直写** control DB。`effective_action` 为 **review / block / captcha**（及 degraded 等非 allow）的事件写入消息总线，由 **AuditIngest** 消费后入库 **`tb_audit_events`** 并供 metrics rollup。

**`effective_action=allow` 不入库**：各层仅追加写入本地 **`*-audit-allow.jsonl`**，**不** `XADD` Stream；edge 经 HTTP ingest 时 control 直接落 allow 日志文件。Stream 中若仍有历史 allow 消息，ingest 返回 `skipped_allow` 并推进 checkpoint，不写 DB。

| 项 | 约定 |
|----|------|
| **默认传输** | **Redis Stream**（`XADD virbius:audit:events`），**仅非 allow** |
...
| 消费组 | `virbius-audit-ingest`；at-least-once；幂等键 `trace_id + rule_id + intercepted_at` |
| allow 备档 | gateway / engine / edge 各写独立 `*-audit-allow.jsonl`（见 §5.8） |
| 非 allow 备档 | PoC 同步写 `gateway-audit.jsonl`、`engine-audit.jsonl` 等 |
...
virbius:
  audit:
    allow-log-path: ${VIRBIUS_DATA_DIR}/audit-allow.jsonl
    gateway-allow-log-path: ${VIRBIUS_DATA_DIR}/gateway-audit-allow.jsonl
    engine-allow-log-path: /tmp/virbius/engine-audit-allow.jsonl
...
R2a 起 engine/gateway 对非 allow 默认走异步 Stream publish；ingest 为 control 侧常驻消费者（checkpoint 表 `tb_audit_ingest_checkpoint`）。

### 5.8 审计中心（运营台）

**入口**：运营台左侧 **「审计中心」**（`/ui` → `ops.html`）。

| 能力 | 说明 |
|------|------|
| 按 trace 查询 | 输入 `trace_id`，合并展示 DB 与 allow 日志 |
| `tb_audit_events` | review / block / captcha 等（ingest 入库） |
| allow 日志 | 读取 gateway / engine / control 三路 `*-audit-allow.jsonl` |
| 文件提示 | 返回各 allow 日志路径及是否可读（PoC 与 control 同机时有效） |

Admin API：

```http
GET /api/v1/admin/tenants/{tenantId}/audit/trace/{traceId}
GET /api/v1/admin/tenants/{tenantId}/audit/ingest-status
```

`audit/trace` 响应字段示例：`trace_id`、`db_events`、`db_count`、`allow_log_events`、`allow_log_count`、`allow_log_files`、`note`。

策略上线页样本表点击 trace 时，弹窗同样调用 `audit/trace`，合并 DB 与 allow 日志行（来源列：`db` / `allow-log`）。

HTTP ingest（edge，`POST /api/v1/internal/audit/events`）：单条 **allow** → 写 `audit-allow.jsonl`（响应 `logged_allow`）；**非 allow** → 异步 `XADD`（响应 `published`）。
...
```text
  ...
  gate_log, metrics      (非 allow)              (Redis Stream / Kafka)
         │                     ▲
         │              *-audit-allow.jsonl（allow，不入库）
         ▼                     │ publish（非 allow）
...
| 区域 | 内容 |
|------|------|
| **左侧导航**（可折叠） | 名单 · 累计 · 请求映射 · **规则**（云 / 管 / 端子菜单）· **策略上线** · **审计中心** |
...
6. **样本表**：audit-samples；点击 trace 弹跨层链路（含 allow 日志，见 §5.8）
7. **租户策略**：租户 `rollout-policy` 编辑（admin；影响本租户全部规则）
8. **操作按钮**：publish / rollback / disable / recover（依状态显示）

**「审计中心」页**：

1. **trace_id 搜索**：查询 `tb_audit_events` + 三路 allow JSONL
2. **DB 事件表**：review / block / captcha 等
3. **allow 日志表**：各层 allow 备档（不入库）
4. **日志文件表**：gateway / engine / edge(control) allow 路径与可读性
...
三端共用：**非 allow audit → ingest → tb_audit_events → metrics_1h → Gate / 看板**；**allow 仅 JSONL 备档**，运营排障经 **审计中心**（§5.8）按 `trace_id` 合并查询。运营 SOP 仍为 §4，不按 layer 分叉。
...
租户级或 App 级下发（**不可 per-rule**）。PoC 方案 B+ 中下列键由 manifest **`sdk_config`** 携带（Control 发布写入）；**不含** `edge_api_key`（Control 拉取 Bearer，仅 **Init** 配置，见 [MVP-OPENSPEC §4.8](./MVP-OPENSPEC.md)）。
...
Redis Stream 用于 engine/gateway **非 allow**：`XADD virbius:audit:events * payload '<json>'`。  
响应示例：`{ "published": 1, "logged_allow": 0, "rejected": 0, "queued_async": true }`；allow 批次为 `{ "logged_allow": N, ... }`。

### 16.6 管/云/端 audit 通道（R3a–R3d）

| layer | 通道 | 状态 |
|-------|------|------|
| cloud | Redis Stream（非 allow）+ jsonl | ✅ R3a：`AuditWriter`；allow → `engine-audit-allow.jsonl` |
| gateway | Redis Stream（非 allow）+ jsonl | ✅ R3a：`gateway-agent` audit；allow → `gateway-audit-allow.jsonl` |
| edge | HTTP batch | ✅ R3d：allow → control `audit-allow.jsonl`；非 allow → Stream |
...
| 模块 | 文件 | 改动 |
|------|------|------|
| virbius-control | **新建** `audit/AuditEventIngestor.java` | ✅；allow → `skipped_allow` |
| virbius-control | `audit/AuditIngestService.java` | ✅（checkpoint；无 DLQ） |
| virbius-control | **新建** `api/InternalAuditController.java` | ✅；allow 落 JSONL |
| virbius-control | **新建** `audit/AuditCenterService.java` | ✅；审计中心 API |
| virbius-control | **新建** `audit/AuditAllowLogWriter.java` | ✅ |
...
| 1.5 | 2026-05-20 | **allow 不入库**（`*-audit-allow.jsonl`）；运营台 **审计中心**；`GET .../audit/trace/{traceId}` |
