...
```text
  ...
  gate_log, metrics        (Redis Stream / Kafka)
         │                     ▲
         ▼                     │ publish
   ops.html Tab           engine / gateway / edge
   「策略上线」
```
...
{
  "pass": false,
  "reasons": ["review_count=42 < min_review_count=100"],
  "metrics": {
    "review_24h": 42,
...
### 8.3 UI（ops.html Tab「策略上线」）

布局：

1. **顶栏**：rule 选择器；badge 显示 rollout_state + canary%
2. **流程条**：draft → dry_run → canary → full 高亮当前步
3. **KPI 卡片**：24h review/block/captcha/hit_rate
4. **图表**：按小时 stacked bar + rollout 变更竖线
5. **Gate 面板**：最近一次 evaluate 结果；「评估升级」「应用」按钮
6. **样本表**：audit-samples；点击 trace 弹链路
...
| G4 | `review_count_24h <= max_review_spike_ratio × baseline_7d_daily_avg` |
